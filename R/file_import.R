...
#' @export
#' @rdname get_og_overlap
#' @examples
#' path <- system.file("extdata", "Orthogroups.tsv.gz", package = "cogeqc")
#' orthogroups <- read_orthogroups(path)
#' ov_df <- get_og_overlap(orthogroups)
get_og_overlap <- function(orthogroups) {

    fog <- unique(orthogroups[, c(1,2)])
    sp_and_ogs <- split(fog$Orthogroup, fog$Species)

    sp <- as.character(unique(fog$Species))
    ov_df <- Reduce(rbind, lapply(combn(sp, 2, simplify = FALSE), function(x) {
        sp1 <- x[1]
        sp2 <- x[2]

        ov <- length(intersect(sp_and_ogs[[sp1]], sp_and_ogs[[sp2]]))
        df <- data.frame(sp1 = sp1, sp2 = sp2, og_overlap = ov)
