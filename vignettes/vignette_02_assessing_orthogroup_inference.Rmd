...
\end{aligned}
$$
The numerator, $homogeneity$, is the product of a weight variable $W_{H}$
and the mean Sorensen-Dice index for all pairwise combinations of genes
in an orthogroup. The Sorensen-Dice index
...
$$
\begin{aligned}
Homogeneity &= W_{H}\left ( \frac{1}{N_{pairs}} \sum_{i=1}^{N_{pairs}} SDI_{i} \right )
\\
\\
...
the same domain, $homogeneity$ will be somewhere between 0 and 1. The
weight variable, $W_{H}$, can be modified by the user, and it equals 1 
by default.
...
large number of smaller subfamilies). It is the product of a weight variable
$W_{D}$ and the sum of the number of orthogroups containing the same 
protein domain scaled by the number of orthogroups. Formally:

$$
\begin{aligned}
Dispersal &= \frac{1}{W_{D}} \left ( \frac{1}{N_{OG}} \sum_{i=1}^{N_{domains}}D_{i} \right ) \\
\\
\end{aligned}
$$
...
convergent evolution. The weight variable, $W_{D}$, equals 1 by default, but it
...
reference orthogroup inference. To do that, you can use the function `compare_orthogroups()`. For example, let's simulate a different orthogroup 
inference by shuffling some rows of the `og` data frame and comparing it to 
...
want to visualize the shape of the distribution in a better way, you can also 
log transform the OG sizes by setting `log = TRUE`.
