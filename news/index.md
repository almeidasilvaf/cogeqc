...
- Added a correction for overclustering in
  [`calculate_H()`](../reference/calculate_H.md) that penalizes protein
  domains in multiple orthogroups.
- Updated vignette to provide a detailed explanation of how homogeneity
...
- Variable **Duplications_50** of the `duplications` data frame was not
  ...
  [`read_orthofinder_stats()`](../reference/read_orthofinder_stats.md)
...
- Added a *max_size* param to
  [`plot_og_sizes()`](../reference/plot_og_sizes.md) to ignore OGs
...
- Reference-based orthogroup inference does not require the exact same
...
- Added functions to explore assembly and annotation statistics in a
  ...
  [`get_genome_stats()`](../reference/get_genome_stats.md),
  [`compare_genome_stats()`](../reference/compare_genome_stats.md), and
  [`plot_genome_stats()`](../reference/plot_genome_stats.md).
